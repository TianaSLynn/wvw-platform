...
| `POST /.netlify/functions/intake` — dispatch table for MHFA-REG-01, MHFA-GRP-01, MHFA-ACC-01, MHFA-ATT-01, MHFA-EVAL-01, MHFA-COMP-01, MHFA-CERT-CORR-01 | Dev-tested, **each path individually feature-flagged off** (`MHFA_REG_01_ENABLED`, `MHFA_GRP_01_ENABLED`, `MHFA_ACC_01_ENABLED`, `MHFA_ATT_01_ENABLED`, `MHFA_EVAL_01_ENABLED`, `MHFA_COMP_01_ENABLED`, `MHFA_CERT_CORR_01_ENABLED`) | Verified: correct routing per form name, unsupported form → 422, regression-checked on every addition that earlier paths still work |
